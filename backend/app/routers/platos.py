...
from app.models import Plato, PlatoIngrediente, PlatoFamiliar, Ingrediente, Familiar
...
def get_plato_detail(db: Session, plato: Plato) -> dict:
    ...
    familiares = []
    for pf in plato.familiares:
        fam = db.query(Familiar).filter(Familiar.id == pf.familiar_id).first()
        if fam:
            familiares.append({"id": fam.id, "nombre": fam.nombre})
    
    return {
        "id": plato.id,
        "nombre": plato.nombre,
        "descripcion": plato.descripcion,
        "momento_dia": plato.momento_dia,
...
def list_platos(skip: int = 0, limit: int = 100, momento_dia: str = None, db: Session = Depends(get_db)):
    query = db.query(Plato)
    if momento_dia:
        query = query.filter(Plato.momento_dia == momento_dia)
...
def create_plato(
    ...
    db_plato = Plato(
        ...
        momento_dia=plato_data.momento_dia,
    )
    db.add(db_plato)
    db.commit()
    db.refresh(db_plato)
    ...
    for familiar_id in plato_data.familiares_ids:
        db_pf = PlatoFamiliar(plato_id=db_plato.id, familiar_id=familiar_id)
        db.add(db_pf)
...
@router.post("/{plato_id}/familiares/{familiar_id}")
def assign_familiar_to_plato(
    plato_id: int, 
    familiar_id: int, 
    db: Session = Depends(get_db),
    admin: Usuario = Depends(require_admin)
    ...
    existing = db.query(PlatoFamiliar).filter(
        PlatoFamiliar.plato_id == plato_id,
        PlatoFamiliar.familiar_id == familiar_id
    ).first()
    if existing:
        raise HTTPException(status_code=400, detail="El familiar ya está asignado al plato")
    
    db_pf = PlatoFamiliar(plato_id=plato_id, familiar_id=familiar_id)
    db.add(db_pf)
    db.commit()
    db.refresh(plato)
    return get_plato_detail(db, plato)
...
@router.delete("/{plato_id}/familiares/{familiar_id}")
def remove_familiar_from_plato(
    plato_id: int, 
    familiar_id: int, 
    db: Session = Depends(get_db),
    admin: Usuario = Depends(require_admin)
):
    db_pf = db.query(PlatoFamiliar).filter(
        PlatoFamiliar.plato_id == plato_id,
        PlatoFamiliar.familiar_id == familiar_id
    ).first()
    if not db_pf:
        raise HTTPException(status_code=404, detail="Familiar no asignado a este plato")
